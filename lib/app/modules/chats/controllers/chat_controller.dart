...
class ChatController extends GetxController {
  var collection = FirebaseFirestore.instance.collection("chats").doc(Get.arguments["docId"]).collection("messages");
  ...
  void feed(QuerySnapshot<Map<String, dynamic>> event) {
    ...
    switch (lastDoc.type) {
      ...
      case DocumentChangeType.modified:
        var messageIndex = messages.indexWhere((element) => element.msgId == lastDoc.doc.id);
        messages[messageIndex].copyWith(value: lastDoc.doc.get("value"), time: lastDoc.doc.get("time"));
        break;
    }
  }
  ...
  Future<void> sendMessage() async {
    ...
    FirebaseFirestore.instance.collection("chats").doc(Get.arguments["docId"]).update({
  ...
  Future<void> takePicture() async {
    ...
    var image = await imagePicker.pickImage(source: imageSource, imageQuality: 70);

    if (image == null) return;

    var id = const Uuid().v4();
    var ref = FirebaseStorage.instance.ref("/chats/${Get.arguments["docId"]}/$id");
  ...
  Future<void> pickFile() async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      allowMultiple: true,
      dialogTitle: "Избери файл който да пратиш на ${"gs"}",
    );
